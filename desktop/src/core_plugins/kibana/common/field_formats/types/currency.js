import {
  ...
  FORMAT_TYPE,
  getFormatter
} from './helper/currency_helper';

export function createCurrencyFormat(FieldFormat) {
  return class CurrencyFormat extends FieldFormat {
    ...
    dynamicFormatter = function (num, precision, locale) {
      ...
      return dynamicIndex > 1 ? locales.options.symbols + ' ' +
            (num / dynamicFormat[dynamicIndex].value).toFixed(precision) +
            ' ' +
    ...
    numberFormatter = function (num, precision, locale, format) {
      ...
      return num > 99999 ? locales.options.symbols + ' ' + (num / formatDesc[0].value).toFixed(precision) +
    ...
    // currency formater object properties
    // id
    static id = 'currency';
    // title to show
    static title = 'Currency';
