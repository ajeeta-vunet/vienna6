const currencyHelper = module.exports = {
  ...
  INPUT_TYPE: [{
    ...
    formats: ['comma', 'lakhs', 'crores', 'millions', 'billions', 'dynamic'],
    dynamic_format: [{
      value: 1E5,
      text: 'Lakhs'
    }, {
  ...
  },
  {
    ...
    formats: ['comma', 'millions', 'billions', 'dynamic'],
    dynamic_format: [{
      value: 1E6,
      text: 'Million'
    }, {
  ...
  getFormatter: function (locale) {
    ...
    const locales = currencyHelper.INPUT_TYPE.find(function (_inputType) {
    ...
    const symbol = locales.options.symbols;
    ...
    const format = function (number, format) {
      let formattedNumber = currencyHelper.toFixed(Math.abs(number), format.decimalPlaces);
      ...
      formattedNumber = format.frontPadding + segment + (typeof splitNumber[1] === 'undefined' ? '' :
        (format.decimal + splitNumber[1])) + format.backPadding;
