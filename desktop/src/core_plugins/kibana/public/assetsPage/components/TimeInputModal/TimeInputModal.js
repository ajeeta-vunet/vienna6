...
export class TimeInputModal extends React.Component {
  ...
  handleSubmit = () => {
    ...
    if(this.state.withinFlag) {
      startDateTime = moment(new Date(this.dateAfterSubtracted(this.state.withinTimeFrameOption,
        this.state.withinTimeFrameValue))).format().slice(0, -6).replace('T', ' ');
      endDateTime = moment(new Date()).format().slice(0, -6).replace('T', ' ');

      appliedTimeFilter = generateHeading(this.props.timeFilter) +
      ' : Last ' + this.state.withinTimeFrameValue + ' ' + this.state.withinTimeFrameOption;
    }else {
      startDateTime = moment(this.state.fromTime).format();
      endDateTime = moment(this.state.toTime).format();
      appliedTimeFilter = generateHeading(this.props.timeFilter) + ' : ' + startDateTime + ' to ' + endDateTime;
    }
    ...
    return(
      <div className="time-input-wrapper">
        ...
        <div className="actions">
          <input className="import-cancel-button" type="button" value="Cancel" onClick={() => this.props.cancelTimeModal()}/>
          <input
