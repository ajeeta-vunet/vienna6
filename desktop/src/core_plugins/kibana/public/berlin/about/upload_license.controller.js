    ...
    $scope.upload = function (file) {
      StateService.updateLicenceDetails(file, Upload).then((response) => {
        ...
        .catch(function (response) {
          ...
          const resonseStatusString = (response.statusText !== undefined) ? response.statusText : 'Some internal server error';
          const responseErrorString = (response.data['error-string'] !== undefined) ? response.data['error-string'] : resonseStatusString;
          $scope.fileUploadErrorMessage = response.data ? responseErrorString : 'Some internal server error';
        });
...
UploadLicenseCtrl.$inject = ['$scope', 'HTTP_SUCCESS_CODE', 'StateService', 'Upload', '$modalInstance', 'Notifier'];
/*eslint-disable*/
