...
class deviceheartbeatAddDataSourceCtrl {
  constructor ($scope, StateService, $uibModalInstance) {
    ...
    $scope.add_dh_device = () => {
        ...
        $scope.new_dh_device.push(new_obj);
    };
    ...
    $scope.saveDataSource = (isValid) => {
      if( isValid ) {
        ...
        $scope.data_obj.standalone_shipper = $scope.standalone_shipper;
            ...
            $scope.data_obj.dh_heartbeat = {};
                 
        $scope.data_obj.dh_heartbeat.dh_interval = $scope.dh_heartbeat.dh_interval;
                    
                 
        $scope.data_obj.dh_heartbeat.dh_count = $scope.dh_heartbeat.dh_count;
                    
                 
        $scope.data_obj.dh_heartbeat.dh_timeout = $scope.dh_heartbeat.dh_timeout;
                    
                 
        $scope.data_obj.dh_heartbeat.dh_size = $scope.dh_heartbeat.dh_size;
                    
                 
        $scope.data_obj.dh_heartbeat.dh_overall_timeout = $scope.dh_heartbeat.dh_overall_timeout;
                    
                 $scope.data_obj.dh_heartbeat.dh_device = $scope.dh_heartbeat.dh_device = $scope.dh_heartbeat.dh_device.concat($scope.new_dh_device);
        ...
        // changed
        if ($uibModalInstance.row) {
            StateService.updateDataSources('deviceheartbeat', $scope.name, $scope.data_obj).then(function() {
            ...
            // Normal add
            StateService.addDataSources('deviceheartbeat', $scope.name, $scope.data_obj).then(function() {
    ...
    $scope.downloadDataSourceAgentConfiguration = (environment) => {
        ...
        data_obj.dh_heartbeat = $scope.dh_heartbeat;
        
        StateService.downloadDataSourceAgentConfiguration('deviceheartbeat', $scope.name, environment, data_obj).then(function() {
         });
     };
     ...
     $scope.downloadDataSourceAgent = (environment) => {
        ...
        StateService.downloadDataSourceAgent('deviceheartbeat', $scope.name, environment, data_obj).then(function() {
         });
     };
