...
import healthbeatDataSourceTemplate from './healthbeatDataSource.html';
import healthbeatAddDataSourceTemplate from './healthbeatAddDataSource.html';

module.directive('healthbeatDataSource', function (StateService, $uibModal) {
  const healthbeatDataSource = {
    restrict: 'E',
    requrie: '^ngModel',
    scope: {
    },
    replace: true,
    template: healthbeatDataSourceTemplate,
    link: (scope) => {
      // Fetch the configuration from backend
      scope.$parent.current_data_source_scope = scope;

      scope.showhealthbeatModal = false;

      scope.healthbeatModalData = {
        title: 'Confirm Delete',
        message: 'Do you want to delete this entry?',
        isForm: false
      };

      StateService.getDataSources('healthbeat').then(function(data) {
      ...
      scope.editConfig = (row, data_received, data_received_step, configuration_step) => {
        ...
        const controller = 'healthbeatAddDataSourceCtrl';

        const modalInstance = $uibModal.open({
          animation: true,
          template: healthbeatAddDataSourceTemplate,
      ...
      scope.deleteDataSourceConfig = (row, index) => {
        ...
        scope.showhealthbeatModal = true;
      };

      scope.onhealthbeatModalSubmit = function () {
        StateService.deleteDataSources('healthbeat', scope.rowToDelete.name).then(function () {
      ...
      scope.onhealthbeatModalClose = () => {
        scope.showhealthbeatModal = false;
      };
    }
  };

  return healthbeatDataSource;
});