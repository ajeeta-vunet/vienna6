...
import logbeatDataSourceTemplate from './logbeatDataSource.html';
import logbeatAddDataSourceTemplate from './logbeatAddDataSource.html';

module.directive('logbeatDataSource', function (StateService, $uibModal) {
  const logbeatDataSource = {
    restrict: 'E',
    requrie: '^ngModel',
    scope: {
    },
    replace: true,
    template: logbeatDataSourceTemplate,
    link: (scope) => {
      // Fetch the configuration from backend
      scope.$parent.current_data_source_scope = scope;

      scope.showlogbeatModal = false;

      scope.logbeatModalData = {
        title: 'Confirm Delete',
        message: 'Do you want to delete this entry?',
        isForm: false
      };

      StateService.getDataSources('logbeat').then(function(data) {
      ...
      scope.editConfig = (row, data_received, data_received_step, configuration_step) => {
        ...
        const controller = 'logbeatAddDataSourceCtrl';

        const modalInstance = $uibModal.open({
          animation: true,
          template: logbeatAddDataSourceTemplate,
      ...
      scope.deleteDataSourceConfig = (row, index) => {
        ...
        scope.showlogbeatModal = true;
      };

      scope.onlogbeatModalSubmit = function () {
        StateService.deleteDataSources('logbeat', scope.rowToDelete.name).then(function () {
      ...
      scope.onlogbeatModalClose = () => {
        scope.showlogbeatModal = false;
      };
    }
  };

  return logbeatDataSource;
});