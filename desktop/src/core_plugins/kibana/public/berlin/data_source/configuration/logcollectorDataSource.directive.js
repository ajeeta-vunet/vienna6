...
import logcollectorDataSourceTemplate from './logcollectorDataSource.html';
import logcollectorAddDataSourceTemplate from './logcollectorAddDataSource.html';

module.directive('logcollectorDataSource', function (StateService, $uibModal) {
  const logcollectorDataSource = {
    restrict: 'E',
    requrie: '^ngModel',
    scope: {
    },
    replace: true,
    template: logcollectorDataSourceTemplate,
    link: (scope) => {
      // Fetch the configuration from backend
      scope.$parent.current_data_source_scope = scope;

      scope.showlogcollectorModal = false;

      scope.logcollectorModalData = {
        title: 'Confirm Delete',
        message: 'Do you want to delete this entry?',
        isForm: false
      };

      StateService.getDataSources('logcollector').then(function(data) {
      ...
      scope.editConfig = (row, data_received, data_received_step, configuration_step) => {
        ...
        const controller = 'logcollectorAddDataSourceCtrl';

        const modalInstance = $uibModal.open({
          animation: true,
          template: logcollectorAddDataSourceTemplate,
      ...
      scope.deleteDataSourceConfig = (row, index) => {
        ...
        scope.showlogcollectorModal = true;
      };

      scope.onlogcollectorModalSubmit = function () {
        StateService.deleteDataSources('logcollector', scope.rowToDelete.name).then(function () {
      ...
      scope.onlogcollectorModalClose = () => {
        scope.showlogcollectorModal = false;
      };
    }
  };
