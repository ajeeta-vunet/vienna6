...
import netflowDataSourceTemplate from './netflowDataSource.html';
import netflowAddDataSourceTemplate from './netflowAddDataSource.html';

module.directive('netflowDataSource', function (StateService, $uibModal) {
  const netflowDataSource = {
    restrict: 'E',
    requrie: '^ngModel',
    scope: {
    },
    replace: true,
    template: netflowDataSourceTemplate,
    link: (scope) => {
      // Fetch the configuration from backend
      scope.$parent.current_data_source_scope = scope;

      scope.shownetflowModal = false;

      scope.netflowModalData = {
        title: 'Confirm Delete',
        message: 'Do you want to delete this entry?',
        isForm: false
      };

      StateService.getDataSources('netflow').then(function(data) {
      ...
      scope.editConfig = (row, data_received, data_received_step, configuration_step) => {
        ...
        const controller = 'netflowAddDataSourceCtrl';

        const modalInstance = $uibModal.open({
          animation: true,
          template: netflowAddDataSourceTemplate,
      ...
      scope.deleteDataSourceConfig = (row, index) => {
        ...
        scope.shownetflowModal = true;
      };

      scope.onnetflowModalSubmit = function () {
        StateService.deleteDataSources('netflow', scope.rowToDelete.name).then(function () {
