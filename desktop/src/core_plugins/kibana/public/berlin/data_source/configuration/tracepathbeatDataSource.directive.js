...
import tracepathbeatDataSourceTemplate from './tracepathbeatDataSource.html';
import tracepathbeatAddDataSourceTemplate from './tracepathbeatAddDataSource.html';

module.directive('tracepathbeatDataSource', function (StateService, $uibModal) {
  const tracepathbeatDataSource = {
    restrict: 'E',
    requrie: '^ngModel',
    scope: {
    },
    replace: true,
    template: tracepathbeatDataSourceTemplate,
    link: (scope) => {
      // Fetch the configuration from backend
      scope.$parent.current_data_source_scope = scope;

      scope.showtracepathbeatModal = false;

      scope.tracepathbeatModalData = {
        title: 'Confirm Delete',
        message: 'Do you want to delete this entry?',
        isForm: false
      };

      StateService.getDataSources('tracepathbeat').then(function(data) {
      ...
      scope.editConfig = (row, data_received, data_received_step, configuration_step) => {
        ...
        const controller = 'tracepathbeatAddDataSourceCtrl';

        const modalInstance = $uibModal.open({
          animation: true,
          template: tracepathbeatAddDataSourceTemplate,
      ...
      scope.deleteDataSourceConfig = (row, index) => {
        ...
        scope.showtracepathbeatModal = true;
      };

      scope.ontracepathbeatModalSubmit = function () {
        StateService.deleteDataSources('tracepathbeat', scope.rowToDelete.name).then(function () {
      ...
      scope.ontracepathbeatModalClose = () => {
        scope.showtracepathbeatModal = false;
      };
    }
  };

  return tracepathbeatDataSource;
});