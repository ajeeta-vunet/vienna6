...
const notify = new Notifier({ location: 'Scan Details' });
...
export class ScheduledScan extends React.Component {
  ...
  convertCronToString = (schedules) => {
    schedules &&
      schedules.map((schedule) => {
        ...
        schedule.cred_list =
          schedule.cred_list && schedule.cred_list.split(',');
        schedule.scheduleFrequency = Object.keys(
          SCHEDULE_FREQUENCY_CONSTANTS
        ).find(
          (key) => SCHEDULE_FREQUENCY_CONSTANTS[key] === schedule.frequency
        );
        if (schedule.scheduleFrequency === undefined)
        {schedule.scheduleFrequency = 'day';}
      });
    return schedules;
  };
  ...
  deleteSelectedItemsForScanList = (rows) => {
    ...
    const deletePromises = Promise.all(
      rows.map((row) => {
        apiProvider.remove(DiscoveryConstants.DELETE_SCAN + row.config_id + '/');
      })
    );
  ...
  onSubmit = (event, configId, scanData) => {
    ...
    const scanNowData = {
      ...
      end_ip: '',
      source_ip:
        typeof scanData.source_ip === 'string'
          ? scanData.source_ip.split(',')
          : scanData.source_ip,
      excluded_ip:
        scanData.excluded_ip.length > 0 ? scanData.excluded_ip.split(',') : [],
    ...
    if (scanData.source_file) {
      ...
      scanNowFormData.append(
        'file',
        scanData.source_file,
        scanData.source_file.name
      );
    } else {
    ...
    if (scanData.run_now) {
      apiProvider.post(DiscoveryConstants.SCAN_NOW, scanNowFormData).then((data) => {
        if (data.statusText === 'OK') {
          notify.info('Scan started successfully');
        } else {
    ...
    const scheduleScanData = {
      ...
      source_ip:
        typeof scanData.source_ip === 'string'
          ...
          : scanData.source_ip,
      name: scanData.schedule_name,
      run_now: scanData.run_now ? scanData.run_now : false,
    };
    ...
    if (event === 'edit') {
      ...
      scheduleScanData.run_now = scanData.run_now ? scanData.run_now : false;
    }

    formData.append('json', JSON.stringify(scheduleScanData));
    if (event === 'add' && scanData.scheduleAt) {
      return apiProvider.post(DiscoveryConstants.SCHEDULE_SCAN, formData).then((data) => {
        if (data.statusText === 'OK') {
          notify.info('Scan scheduled successfully');
        } else {
          notify.error(`${data.response.data['error-string']}`);
        }
      });
      ...
      return apiProvider
        .put(DiscoveryConstants.EDIT_SCHEDULED_SCAN + configId + '/', formData)
        .then((data) => {
          if (data.statusText === 'OK') {
            notify.info('Scan edited successfully');
          } else {
            notify.error(`${data.response.data['error-string']}`);
          }
        });
  ...
  onRowAction = (e, configId) => {
    apiProvider.post(DiscoveryConstants.SCAN_AGAIN + configId + '/').then((data) => {
      if (data.statusText === 'OK') {
        notify.info('Scan started successfully');
      } else {
        notify.error(`${data.response.data['error-string']}`);
  ...
  validateValue = (key, value) => {
    return this.state.listOfScheduledScans &&
      this.state.listOfScheduledScans.find(
  ...
  render() {
    const scheduleScanMeta = {
      ...
      headers: ['Name', 'Frequency', 'Seed IP', 'Source IP'],
      rows: ['schedule_name', 'frequency', 'seed_ip', 'source_ip'],
