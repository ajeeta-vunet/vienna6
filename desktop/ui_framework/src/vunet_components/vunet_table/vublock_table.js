...
import './_vunet_vublock_table.less';

import { VunetModal } from '../vunet_modal/vunet_modal';


export class VunetVublockTable extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  */
  componentWillReceiveProps(newProps) {
    ...
    // If wrapTableCellContents is set to false, remove the box shadow on table.
    ...
    if (filter === '') {
      ...
      const pageOfItems = this.rows.filter(item => {
        return this.props.metaItem.rows.some(row => {
          ...
          if (!_.isArray(item[row.name]) && !_.isObject(item[row.name]) && (!_.isEmpty(item[row.name]) ||
            _.isNumber(item[row.name]) || _.isBoolean(item[row.name]))
          && !_.isUndefined(item[row.name])) {
            if (_.isNumber(item[row.name])) {
              return item[row.name].toString().indexOf(filter) > -1 ? true : false;
            }
            // Convert value to lowercase for case insensitive filter search.
            return item[row.name].toLowerCase().indexOf(filter.toLowerCase()) > -1 ? true : false;
          }
        });
      });
  ...
  */
  renderRowCells(item) {
    ...
    this.props.metaItem.rows.forEach(row => {
      ...
      if (typeof (item[row.name]) === 'object' && item[row.name].image) {
        cells.push({ value: <img src={item[row.name].image}/> });
      } else if (row.type === 'link') {
        cells.push(
          {
            value:
  <a
    href={`/app/vienna#/berlin/data_source/vuBlock_Store/modify/${item.vublock_id}`}
  ...
  >
    {item[row.name]}
      ...
      else if (row.type === 'progressbar') {
        cells.push({ value: <ProgressBar width={item[row.name]} message={item[row.name] + '%'} /> });
      } else {
        if (this.props.metaItem.columnData) {
          const columnData = this.props.metaItem.columnData.find(cd => cd.columnName === row.name);
          if (columnData) {
            cells.push({ value: columnData.func(row, item[row.name]) });
          } else {
            cells.push({ value: item[row.name] });
          }
        } else {
          const tooltip = row + '_tooltip';
          cells.push({ value: item[row.name], name: row.name, [tooltip]: item[tooltip] });
        }
      }
    });
    ...
    if (newSelectedIds === 'all') {
      if (this.refs.selectAll.checked) {
        rows.map(row => {
          ...
          //this.setState({ selectedRowIds: this.rowIds });
        });
      }
      else { this.rowIds = []; }
    }
    else {
      const selectedRowIndex = this.rowIds.indexOf(newSelectedIds);
      if (selectedRowIndex >= 0) {
        this.rowIds.splice(selectedRowIndex, 1);
      }
      else
      {
        this.rowIds.push(newSelectedIds);
      }
    }
    this.setState({ selectedRowIds: this.rowIds });
  };
    ...
    if (event === 'edit') {
      if (this.props.metaItem.isFormWizard) {
        ...
        this.setState({
          showAddEditModal: true,
          formModal: {
            title: 'Edit' + (this.props.metaItem.title ? '  ' + this.props.metaItem.title + ' ' + rowId : ''),
            action: 'edit',
            restApiId: restApiId,
            name: rowId,
            isFormWizard: this.props.metaItem.isFormWizard,
        ...
        const _editData = this.rows.find(item => item[this.props.metaItem.id] === rowId);
        this.setState({
          showAddEditModal: true,
          editId: rowId,
          formModal: {
      ...
      if (event === 'delete') {
        ...
        const editedData = this.rows.find(item => item[this.props.metaItem.id] === rowId);
  ...
  */
  renderNestedTable() {
    ...
    const toolbar = this.props.metaItem.add ?
      (
        <KuiToolBar>
          <KuiToolBarSearchBox onFilter={this.filterItems} />
          <KuiToolBarSection>
            <KuiListingTableCreateButton
              key="create"
              onCreate={this.onCreate.bind(this)}
            ...
            <KuiListingTableDeleteButton
              ...
              onDelete={this.onDelete}
              aria-label="Delete selected"
              disabled={this.state.selectedRowIds.length === 0 ? true : false}
            />
            <select
              className="kuiSelect"
              key="value"
    ...
    const footerbar = this.props.metaItem.add ?
      (
        <KuiToolBarFooter>
          <KuiToolBarFooterSection>
            <KuiPager
    ...
    const tableBody = rows.map((row, index) => {
      ...
      return (
        <KuiTableBody key={index}>
          ...
          >
            <KuiTableRowCell
              className="kuiTableRowCell--checkBox"
            >
              <input
                type="checkbox"
                className="kuiCheckBox"
                checked={this.state.selectedRowIds.indexOf(row.id) >= 0}
                onChange={() => this.onItemSelectionChanged(row.id)}
              />
            </KuiTableRowCell>
    ...
    const nestedTable = (
      <KuiTable className="outer-table">
        <KuiTableHeader>
          <KuiTableHeaderCell
            className="kuiTableHeaderCell--checkBox"
          >
            <input
              type="checkbox"
              className="kuiCheckBox"
              ref="selectAll"
              checked={rows.length > 0 && rows.length === this.state.selectedRowIds.length}
              onChange={() => this.onItemSelectionChanged('all')}
            />
          </KuiTableHeaderCell>
          <KuiTableHeaderCell
  ...
  hasSubTable() {
    if (this.props.metaItem.pagination) {
      return true;
    }
    else
    {
  ...
  renderTable() {
    //
    if (this.props.metaItem.hasSubTable) { return this.renderNestedTable(); }
    else {
      return (<KuiListingTable
        pager={this.props.metaItem.pagination ? this.renderPager() : null}
        toolBarActions={this.props.metaItem.toolbar ? this.renderToolBarActions() : null}
        search={this.props.metaItem.search}
        selection={this.props.metaItem.selection}
        footerbar={this.props.metaItem.footerbar}
        toolbar={this.props.metaItem.toolbar}
        ...
        wrapTableCellContents={this.props.metaItem.wrapTableCellContents}
